   ...
   // __android_log_print(ANDROID_LOG_DEBUG, "TypeMove"," %d",TypeMove);
    ...
    switch(TypeMove)
    {
        case 0:
            ...
            newValue = myShip->posX + (1 * myShip->speed);
           if(newValue < widthScreen)
        ...
        case 1:
            ...
            newValue = myShip->posY + (1 * myShip->speed);
            if(newValue < heightScreen)
...
void moveMyShipGeneral(float * accelValues,int sizeAccelValues ,UserShip * myShip,int widthScreen, int heightScreen)
{
    ...
    for(index = 0 ; index < sizeAccelValues ; index++)
    {
        switch(index)
        {
            case 0:
                if(accelValues[index] > 0.15)
                    moveMyShip(myShip,0,widthScreen,heightScreen);
            
                if(accelValues[index] < -0.15)
                    moveMyShip(myShip,2,widthScreen,heightScreen);
       ...
       //         sprintf(values,"value X %f",accelValues[index]);
            ...
            case 1:
                if(accelValues[index] > 0.15)
                    moveMyShip(myShip,1,widthScreen,heightScreen);

                if(accelValues[index] < -0.15)
                    moveMyShip(myShip,3,widthScreen,heightScreen);
